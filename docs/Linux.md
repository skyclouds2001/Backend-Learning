...
检查指定的网络服务器是否可联通 `ping [-c num] <IP或主机名>`
-c 指定ping的次数，默认无限次
...
端口
- 物理端口 | 虚拟端口
- 公认端口 1~1023 系统内置或知名程序预置
- 注册端口 1024~49151 松散地绑定程序或服务
- 动态端口 49152~65535 用于程序对外进行网络链接时使用
- 22 端口 SSH服务使用
...
用户ID 进程ID 进程父ID CPU占用比 进程启用时间 启用进程的终端序号 进程占用CPU时间 启动路径或启动命令
